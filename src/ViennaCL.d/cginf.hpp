...
namespace detail
{
  ...
  */
  ...
  template<typename VectorT>
  double norm_inf(VectorT const & red)
  {
    double R = 0.0;
    int n = red.size();
    for (int i = 0; i<n; i++){
                     ...
                     void *monitor_data = NULL)
  {
    ...
    for (unsigned int i = 0; i < tag.max_iterations(); ++i)
    {
      ...
      if (monitor && monitor(result, std::sqrt(std::fabs(new_ipp_rr_over_norm_rhs)), monitor_data))
        break;
      if (std::fabs(new_ipp_rr_over_norm_rhs) < tag.tolerance() *  tag.tolerance() || std::fabs(new_ip_rr) < tag.abs_tolerance() * tag.abs_tolerance())
	{ //squared norms involved here
	  // tsukud-y
	  if( viennacl::linalg::norm_inf(residual) <= 1.0e-5 ) break;
	}
