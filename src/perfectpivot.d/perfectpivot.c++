...
// 連立方程式行列を表示
static void printEq(sparse::matrix<double>&a, vector<double>&b,int n){
  int   i,j;
...
// 完全ピボット選択
static double perfectsel(
		  sparse::matrix<double>&a, // 連立方程式の係数行列：[N*N]
		  vector<double>&b, // 連立方程式の右辺：[N]
		  vector<int>&ib,   // 変数整列インデクス[N]
		  int     n,  // 連立方程式の元数
		  int     k)  // ピボットインデクス
{
...
static int gausselim(
	      sparse::matrix<double>&a,  // 連立方程式の係数行列：[N*N]
	      vector<double>&b,  // 連立方程式の右辺：[N]
	      vector<double>&x)// 連立方程式の解ベクトル：[N]
	      
{
  int n = a.size();
  vector<int> ib(b.size()); // 未知数整列インデクス
...
vector<double> perfectpivot(sparse::matrix<double>&a, vector<double>&b)
{
  ...
  vector<double>xx(a.size()-1);
  vector<double>bb(a.size()-1);  

  for ( int i=1; i<a.size(); i++ ) {
    for (auto it: a[i] ) {
      int j = it.first;
      aa[i-1][j-1] = it.second;
    }
  }
...
int main(){
  int N = 6;
  sparse::matrix<double> a(N); // 連立方程式の係数行列：[N*N]
  vector<double> b(N);         // 連立方程式の右辺：[N]
  vector<double> x(N);         // 連立方程式の解ベクトル：[N]
  int    n = a.size()-1;         // 連立方程式の元数
  int     err;
  int     i,j,k;
  double  xv;
